...
- `Advisor`：只给出模型建议，不执行动作。
- `Combat Auto`：只允许模型执行战斗出牌和结束回合；地图、奖励、商店、事件仍只显示建议。
- 当前支持 OpenAI-compatible Chat Completions 接口。
- API Key 写入本机 `AI_Training/model_config.json`，该文件不会提交到 Git。
...
- LLM 决策接入已具备第一版：配置、建议、战斗自动执行、校验和日志记录。
