...
- 自动 `git fetch` 和 `git pull --ff-only`，只做安全的快进更新。
- 如果你有已跟踪的本地改动，会先自动做一个 Git stash 备份，避免直接拉取失败。
- 如果 `requirements.txt` 变了，会自动同步 Python 依赖。

建议先关闭控制台和 AI，再执行更新。
如果脚本提示“本地和远端都改了”或“Git 正在 merge/rebase”，它会停下来，不会替你冒险做合并。

如果控制台已经打开，也可以直接在网页左侧的 **仓库更新** 面板里点 **一键更新仓库**。
网页更新完成后，建议关闭当前控制台，再重新运行 `start_all.bat`，确保加载到最新前端和后端代码。
